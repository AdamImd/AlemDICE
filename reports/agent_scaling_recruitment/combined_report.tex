...
E1, the E2b v4 Open-only confirmation, and E3 remain prospective.  E2b v2 and
v3 made bounded hosted attempts and are preserved as poisoned,
non-promotable diagnostics rather than efficacy comparisons.  V3 eliminated
the earlier output truncation but produced a negative Mutual-Nomination
mechanism screen; v4 therefore confirms only Open Volunteer.  A
...
E2: RecruitmentArena-6 & \pass{} provider-free / \fail{} v2--v3 / \pending{} v4 &
...
hosted calls; the v3 canary failed after 40.  The Open-only v4 amendment has
passed fake orchestration only.\\
...
\subsection{E2b v3 negative Mutual screen and prospective Open-only v4}
...
V4 is prospectively frozen as Open Volunteer with the truth-free joint exact
allocator, retaining the model, high reasoning, 4,096-token allowance,
...
This validates orchestration only; no hosted v4 call has been made.
...
\begin{itemize}
  ...
  cannot establish that Open is superior to a repaired or redesigned Mutual
  mechanism.
  ...
  \code{fsync} latency.  The v4 fake 272-call lifecycle created 544 anchors;
...
The only completed efficacy conclusion remains engineering.  Alem's canonical
language-agent evaluation path supports one, two, three, four, and six
physical agents under the tested provider-free conditions.  E0 passed its
short compatibility gate.  E0.1 passed every structural gate but failed the
literal full-horizon requirement because three inactive-policy episodes ended
naturally.  Separately, E2b provides a bounded negative mechanism diagnostic:
raising the output allowance removed truncation, but frozen Mutual Nomination
still produced repeated self-omission and a true-infeasible lock.  This does
not establish a population-scale coordination effect or Open superiority.
