...
The completed evidence includes two provider-free engineering studies and a
bounded hosted six-agent recruitment mechanism screen.
...
E1 and E3 remain prospective.  E2b v2 and v3 are preserved as poisoned,
non-promotable diagnostics: the larger v3 output allowance removed the
observed truncation, but frozen Mutual Nomination still produced repeated
self-omission and a true-infeasible lock.  The prospectively frozen Open-only
v4 confirmation then completed all 12 cells.  It achieved full reward and
oracle-allocation coverage in 12/12 episodes, with 15/15 true-feasible locks,
two successfully repaired semantic-invalid decisions, and no transport error,
incomplete response, or max-output truncation in 293 calls.  Reward was not
perfect allocation: 10/12 whole allocations exactly matched the true oracle,
with mean utility regret 0.005660 and mean raw-cost delta $+4.9167$.
A provider-free E2d2 replication remains the allocation reference under
scripted truthful public claims.  Together these results promote only Open
Volunteer with truth-free joint exact allocation to a bounded paired E3 Alem
transfer behind an opt-in topology while preserving Source action behavior.
They do not establish Open superiority, embodied task efficacy, population
...
Open/joint-exact is promoted to a bounded paired transfer; opt-in,
receiver-specific communication routing and Alem task binding remain to be
...
E1 and E3 remain necessary.  The preregistered sequence is designed to prevent
a routing, formatting, hidden-information, or population confound from being
mistaken for collective intelligence.  Downstream claims must continue to be
updated from canonical artifacts rather than console summaries.
