...
The remaining studies are preregistered and have no results at the time of this
report.  E1 will measure the unchanged GPT-5.4 nano Source baseline as physical
population changes.  E2 will isolate decentralized recruitment in a
deterministic six-agent microbenchmark with bounded public control messages and
team-private conversation.  E3 will transfer the promoted mechanism to Alem
...
E2: RecruitmentArena-6 & \pending &
TFP1, arena, recruitment policies, selectors, oracle, and LLM mechanism studies
must be implemented and gated.\\
...
At the 23 July 2026 eleven-cell audit checkpoint, all eleven completed
artifacts satisfied this contract.  Ten had one provider attempt per logical
response.  The $N=4$, seed 13101 artifact had 800 completed decisions, 801
provider attempts, one \code{APIConnectionError} assigned to worker 2, zero
incomplete responses, and no lost call.  Including the clean preflight, the
campaign had used 5,174 of 12,001 logical responses and 5,175 of 15,000
provider attempts.  This is an accounting result, not yet a behavioral
efficacy conclusion; the incomplete 11/15 grid remains explicitly
watermarked.
...
At the time of this report, E2/E3-specific runtime code, tests, launchers, and
configs are not present.  Existing infrastructure is useful but insufficient:
the evaluator supplies parallel calls, one-tick broadcast, route accounting,
and canonical artifacts; E0 supplies six-agent compatibility evidence; and the
existing DCP1 parser provides a strict-message pattern.  DCP1 does not implement
task cards, team membership, leases, byte caps, or receiver-specific routing
and must not be relabeled as TFP1.
